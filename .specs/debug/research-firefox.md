...
- RDP is organised around server-side _actors_; every packet contains either a `to` field (client ➝ server) or `from` field (server ➝ client), and actors are responsible for replying to requests to keep conversations in sync ([protocol actors](https://firefox-source-docs.mozilla.org/devtools/backend/protocol.html#actors)).
