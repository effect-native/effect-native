...
Captured: 2026-02-24 18:53:30 EST
Branch: feat/sqlite-graph-ext-accelerator
HEAD: ca7b8234c02a0cf641728990598de95738219e52
Bun: 1.3.5
Node: v24.12.0
System: Darwin arm64
...
- Package metadata now targets npm publication as `@effect-native/sqlite-graph` with `private: false` and `publishConfig.access: public`.
...
- Demo now uses package-level Bun bootstrap helper (`@effect-native/sqlite-graph/bun`, `withBunGraphRuntime`) and no longer inlines `setCustomSQLite` or `loadExtension` ritual code.
...
- Added Bun runtime helper module `packages/sqlite-graph-ext/src/bun.ts` and subpath export `@effect-native/sqlite-graph/bun`.
