...
```bash
bun add @effect-native/debug effect @effect/platform @effect/platform-node
```
...
**See it in action**: The `steps` CLI tool (`packages/debug/src/cli/steps.ts`)
...
**See**: `packages/debug-demos/WORKERS-MEMORY-GUIDE.md` for comprehensive guide
...
- **Demos**: `packages/debug-demos/` - Memory leak detection demos
- **Guides**:
  - Node.js leak hunting: `packages/debug-demos/BLOG-POST.md`
  - Workers debugging: `packages/debug-demos/WORKERS-MEMORY-GUIDE.md`
