...
fn appendSqlLiteral(allocator: std.mem.Allocator, output: *std.ArrayList(u8), value: []const u8) !void {
  try output.append('\'')
  for (value) |byte| {
    if (byte == '\'') {
...
fn buildInClauseFromSet(allocator: std.mem.Allocator, values: []const []const u8) ![]const u8 {
  ...
  for (values, 0..) |value, idx| {
    ...
    try appendSqlLiteral(allocator, &out, value)
  }
  return out.toOwnedSlice()
}

fn appendSetText(output: *std.ArrayList(u8), values: []const []const u8) !void {
  for (values, 0..) |value, idx| {
...
fn appendNullableIntText(output: *std.ArrayList(u8), value: ?c.sqlite3_int64) !void {
  if (value) |v| {
...
fn columnText(statement: *c.sqlite3_stmt, idx: c_int) []const u8 {
  const ptr = c.sqlite3_column_text(statement, idx)
  if (ptr == null) return ""
  const len = c.sqlite3_column_bytes(statement, idx)
  return @as([*]const u8, @ptrCast(ptr))[0..@intCast(usize, len)]
}
...
export fn graph_out_many(context: ?*c.sqlite3_context, argc: c_int, argv: [*]?*c.sqlite3_value) callconv(.C) void {
  ...
  if (edgeType.len > 0) {
    ...
    appendSqlLiteral(allocator, &sql, edgeType) catch {
      emitError(context, "graph_out_many failed")
      return
    }
  ...
  const sqlZ = std.fmt.allocPrintZ(allocator, "{s}", .{sqlText}) catch {
  ...
  while (true) {
    ...
    if (step == c.SQLITE_ROW) {
      const src = blk: {
        ...
        break :blk @as([*]const u8, @ptrCast(ptr))[0..@intCast(usize, len)]
      }
      const dst = blk: {
...
export fn graph_out_idset(context: ?*c.sqlite3_context, argc: c_int, argv: [*]?*c.sqlite3_value) callconv(.C) void {
  ...
  var currentValues = std.ArrayList([]const u8).init(allocator)
  ...
  while (true) {
    ...
    if (step == c.SQLITE_ROW) {
      ...
      if (currentSrc == null or !std.mem.eql(u8, currentSrc.?, src)) {
        ...
        currentValues = std.ArrayList([]const u8).init(allocator)
...
export fn graph_two_hop_counts(context: ?*c.sqlite3_context, argc: c_int, argv: [*]?*c.sqlite3_value) callconv(.C) void {
  ...
  appendSqlLiteral(allocator, &sql, hop1EdgeType) catch {
    emitError(context, "graph_two_hop_counts failed")
    return
  }
  ...
  appendSqlLiteral(allocator, &sql, hop2EdgeType) catch {
    emitError(context, "graph_two_hop_counts failed")
    return
  }
...
export fn graph_two_hop_supporters(context: ?*c.sqlite3_context, argc: c_int, argv: [*]?*c.sqlite3_value) callconv(.C) void {
  ...
  var output = std.ArrayList(u8).init(allocator)
  var currentDst: ?[]const u8 = null
  var currentSupporters = std.ArrayList([]const u8).init(allocator)
  ...
  while (true) {
    ...
    if (step == c.SQLITE_ROW) {
      ...
      if (currentDst == null or !std.mem.eql(u8, currentDst.?, dst)) {
        ...
        currentSupporters = std.ArrayList([]const u8).init(allocator)
      }
...
export fn ranked_diff(context: ?*c.sqlite3_context, argc: c_int, argv: [*]?*c.sqlite3_value) callconv(.C) void {
  ...
  appendSqlLiteral(allocator, &sql, oldSerpId) catch {
    emitError(context, "ranked_diff failed")
    return
  }
  ...
  appendSqlLiteral(allocator, &sql, newSerpId) catch {
    emitError(context, "ranked_diff failed")
    return
  }
  sql.appendSlice(")\n unified AS (\n") catch {
