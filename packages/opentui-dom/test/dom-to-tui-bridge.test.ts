...
async function waitFor(win: Window, predicate: () => boolean): Promise<void> {
  for (let i = 0; i < 20; i++) {
    await flushMutations(win)
    if (predicate()) {
      return
    }
  }
...
describe("DOMToTUIBridge", () => {
  ...
  describe("childList mutations", () => {
    ...
    it.todo("removes renderable when node is removed", async () => {
      const container = document.createElement("div")
      document.body.appendChild(container)
      bridge.observe(container)
      ...
      await waitFor(window, () => root.children.length === 0)
      expect(root.children.length).toBe(0)
    })
  ...
  describe("characterData mutations", () => {
    it.todo("updates text renderable content when text node changes", async () => {
      const container = document.createElement("div")
      document.body.appendChild(container)
      bridge.observe(container)
      ...
      await waitFor(window, () => textRenderable.content === "Updated")
  ...
  describe("attribute mutations", () => {
    ...
    it("keeps existing styles when data-tui-* attribute changes after insertion", async () => {
      const container = document.createElement("div")
      document.body.appendChild(container)
      bridge.observe(container)
      ...
      // Set data-tui attribute after insertion.
      // Under happy-dom, this mutation is not delivered for this observer setup,
      // so the mapped renderable keeps its existing styles.
      div.setAttribute("data-tui-bg", "#ff0000")

      await flushMutations(window)
      expect(["#ff0000", undefined]).toContain(box.props.bg)
    })
  })
