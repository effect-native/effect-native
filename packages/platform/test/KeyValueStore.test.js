...
export const testLayer = (layer) => {
    ...
    afterEach(() => run(Effect.gen(function* () {
        const kv = yield* (KeyValueStore.KeyValueStore);
        yield* (kv.clear);
    })));
    it("set", () => run(Effect.gen(function* () {
        const kv = yield* (KeyValueStore.KeyValueStore);
    ...
    it("get/ missing", () => run(Effect.gen(function* () {
        const kv = yield* (KeyValueStore.KeyValueStore);
        yield* (kv.clear);
        const value = yield* (kv.get("foo"));
        assertNone(value);
    })));
    it("remove", () => run(Effect.gen(function* () {
        const kv = yield* (KeyValueStore.KeyValueStore);
    ...
    it("clear", () => run(Effect.gen(function* () {
        const kv = yield* (KeyValueStore.KeyValueStore);
        yield* (kv.set("foo", "bar"));
        yield* (kv.clear);
        const value = yield* (kv.get("foo"));
    ...
    it("modify", () => run(Effect.gen(function* () {
        const kv = yield* (KeyValueStore.KeyValueStore);
        yield* (kv.set("foo", "bar"));
    ...
    it("modify - none", () => run(Effect.gen(function* () {
        const kv = yield* (KeyValueStore.KeyValueStore);
        const value = yield* (kv.modify("foo", (v) => v + "bar"));
        const length = yield* (kv.size);
        assertNone(value);
...
describe("KeyValueStore / prefix", () => {
    it("prefixes the keys", () => Effect.gen(function* () {
...
const runUserStore = (effect) => Effect.runPromise(Effect.provide(effect, UserStore.layer.pipe(Layer.provide(KeyValueStore.layerMemory))));
describe("KeyValueStore / SchemaStore", () => {
    it("encodes & decodes", () => runUserStore(Effect.gen(function* () {
    ...
    it("prefix", () => runUserStore(Effect.gen(function* () {
        const store = yield* (UserStore.tag);
        const prefixed = KeyValueStore.prefix(store, "prefix/");
...
//# sourceMappingURL=KeyValueStore.test.js.map