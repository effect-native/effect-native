...
import { MiniDomError, Transaction } from "@effect-native/minidom"
...
describe("MiniDom.Transaction capability (FR1.11 / SC7.8 / H14)", () => {
  it.effect("propagates conflicts via withTransaction", () =>
    Effect.gen(function*() {
      ...
      const passthrough = Transaction.make((operation) => operation)

      const attempt = yield* Transaction.run(passthrough, Effect.fail(conflict)).pipe(Effect.either)
      ...
      const unsupported = Transaction.unsupported({ message: "adapter missing transaction support" })
      const unsupportedAttempt = yield* Transaction.run(unsupported, Effect.void).pipe(Effect.either)
