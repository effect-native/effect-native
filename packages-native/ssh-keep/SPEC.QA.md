...
**QA-25: Entrypoint Definition Ambiguity** - NEW  
Status: Critical  
The spec defines "entrypoint" as "a single user-facing command invoked on the thin client" but also refers to `~/.local/bin/ssh-keep` as "executable entrypoint used *on the server*". This creates confusion about whether there are one or two entrypoints and their relationship.

**QA-26: "Single Stable Entrypoint" Implementation** - NEW  
Status: Underspecified  
B1 requires a "single command" but doesn't specify if this is:
- A shell alias the user must define
- A script they must install locally
- A command pattern they must remember
The implementation requires Bun runtime locally, which contradicts the "single stable" aspect.
...
**QA-27: Thin Client Command Distribution** - NEW  
Status: Critical gap  
...
**QA-28: Host Selection Scope Boundary** - NEW  
Status: Underspecified  
...
**QA-29: SSH Key/Auth Failure Handling** - NEW  
Status: Unaddressed  
...
**QA-30: Tmux Server Death Recovery** - NEW  
Status: Critical edge case  
...
**QA-31: Compatibility Assumptions Verification** - NEW  
...
**QA-32: Security Invariants Enforcement** - NEW  
