...
describe("GhosttyHarness", () => {
  ...
  describe("terminal operations", () => {
    ...
    test("handles cursor positioning", async () => {
      ...
      await harness.writeAsync(term, "\x1b[3;5HX")
    ...
    test("clears screen", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "Hello")
      await harness.writeAsync(term, "\x1b[2J\x1b[H") // Clear and home
      await harness.writeAsync(term, "World")
  ...
  describe("ANSI color handling", () => {
    test("different colors have different fgColor values", async () => {
      ...
      await harness.writeAsync(term, "\x1b[31mRED\x1b[32mGREEN\x1b[34mBLUE\x1b[0m")
    ...
    test("256-color and default have different values", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "\x1b[38;5;196mRED256\x1b[0m DEFAULT")
    ...
    test("RGB/truecolor sets distinct foreground color", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "\x1b[38;2;255;128;64mRGB\x1b[0m DEFAULT")
    ...
    test("background colors are preserved", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "\x1b[48;2;255;0;0mRED_BG\x1b[0m DEFAULT")
  ...
  describe("text attributes", () => {
    test("preserves bold", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "\x1b[1mBOLD\x1b[0m normal")
    ...
    test("preserves italic", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "\x1b[3mITALIC\x1b[0m normal")
    ...
    test("preserves underline", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "\x1b[4mUNDER\x1b[0m normal")
    ...
    test("preserves faint/dim", async () => {
      const term = harness.createTerminal(40, 5)
      await harness.writeAsync(term, "\x1b[2mFAINT\x1b[0m normal")
    ...
    test("preserves combined attributes", async () => {
      ...
      await harness.writeAsync(term, "\x1b[1;3;4;31;43mCOMBO\x1b[0m NORMAL")
  ...
  describe("screenshot sanitization", () => {
    ...
    test("large terminal has no garbage characters", async () => {
      ...
      await harness.writeAsync(term, "Small content in large terminal")
      const screenshot = harness.screenshot(term)
      expect(screenshot).not.toMatch(GARBAGE_PATTERN)
    })
  })
